...
void nwkmap_init(unsigned int& context_id)
{
    ...
    auto iter = std::find_if(begin(net_bypasses), end(net_bypasses), 
        [](const std::pair<std::unique_ptr<nwbp::Network_bypass>, std::string>& _obj) -> bool {
            return !_obj.first;
        });
    /* Create net_bypass */
    std::unique_ptr<nwbp::Network_bypass> ptr_to_net = std::make_unique<nwbp::Network_bypass>();
    if (iter == end(net_bypasses)) {
        net_bypasses.push_back(std::make_pair(std::move(ptr_to_net), ""));
        context_id = static_cast<unsigned int>(net_bypasses.size() - 1);
    }
    else {
        context_id = static_cast<unsigned int>(std::distance(begin(net_bypasses), iter));
        net_bypasses[context_id].first = std::move(ptr_to_net);
    }
}
...
{
    std::lock_guard<std::mutex> g{mutex};
    if (context_id < net_bypasses.size() && net_bypasses[context_id].first.get()) {
...
void nwkmap_exit(const unsigned int& context_id)
{
    std::lock_guard<std::mutex> g{mutex};
    if (context_id < net_bypasses.size() && net_bypasses[context_id].first.get()) {
        net_bypasses[context_id].first.reset();
